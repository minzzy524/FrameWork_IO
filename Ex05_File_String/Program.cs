...
namespace Ex05_File_String
{
    //Stream : IO 최상위 클래스(읽기 , 쓰기 , 검색등의 작업)

    //*** FileStream : 바이트단의 전송을 통한 파일의 입출력 // 이걸로 파일 생성

    //bufferStream : 버퍼는 메모리의 바이트 블록으로서 데이터를 캐시하여
    // 운영 체제에 대한 호출 수를 줄이는 데 사용됩니다. 따라서 
    // 버퍼는 읽기 및 쓰기 성능을 향상시킵니다. 
    //읽기 또는 쓰기에 버퍼를 사용할 수 있지만 둘 모두에 동시에 사용할 수는 없습니다.

    //MemoryStream :MemoryStream 클래스에서는 디스크나 네트워크 연결 // 이건 잘 안 써(배열이랑 비슷하니까 굳이 안 씀)
    //대신 메모리를 백업 저장소로 사용하는 스트림을 만듭니다. 


    //StringReader & StringWriter 클래스는 문자열을 스트림에 기록하거나
    //읽어낼 때 사용하는 클래스(목표지점이 string형의 데이터)  // 이걸로 파일 읽음

    //StreamReader와 StreamWirter 클래스는 바이트 스트림을 문자스트림으로
    //바꾸어 주는 역활을 담당하는 스트림입니다 (FileStream 이랑 짝 지어서 씀)
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //default  경로 (실행파일)
            //D:\Ecount\Labs\FrameWork_IO\Ex05_FileStream\bin\Debug\hello.txt
